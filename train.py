...
for epoch in range(opt.epochs):
    ...
    with torch.no_grad():
        for batch_img1, batch_img2, labels in val_loader:
            ...
            cd_val_report = prfs(labels.data.cpu().numpy().flatten(),
                                ...
                                #  average ='binary',)
                                 average='macro',)
                                #  pos_label=1)
            # print( cd_val_report.average(),'1111111111111111111111111111111111')
        ...
        if ((mean_val_metrics['cd_precisions'] > best_metrics['cd_precisions'])
            ...
            # Save model and log
            if not os.path.exists('./tmp-20221104-xviewalldata-cutmix-2-ECEM-0.001'):
                os.mkdir('./tmp-20221104-xviewalldata-cutmix-2-ECEM-0.001')
            with open('./tmp-20221104-xviewalldata-cutmix-2-ECEM-0.001/metadata_epoch_' + str(epoch) + '.json', 'w') as fout:
                json.dump(metadata, fout)

            torch.save(model, './tmp-20221104-xviewalldata-cutmix-2-ECEM-0.001/checkpoint_epoch_'+str(epoch)+'.pt')
