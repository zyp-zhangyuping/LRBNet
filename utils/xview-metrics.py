...
class PathHandler:
    def __init__(self, pred_dir:Path, targ_dir:Path, img_id:str):
        ...
        temp = glob.glob(os.path.join(pred_dir, "*test_{}.png".format(img_id)))
        self.lp = temp[0] if len(temp) > 0 else None # localization prediction
        temp = glob.glob(os.path.join(pred_dir, "*test_{}.png".format(img_id)))
        self.dp = temp[0] if len(temp) > 0 else None # damage prediction
        temp = glob.glob(os.path.join(targ_dir, "*test_{}.png".format(img_id)))
    ...
    def load_and_validate_image(self, path):
        ...
        img = np.array(Image.open(path).convert('L')) //50 #numpy形式  50  100 
        # print('--------------------------------')
        # print(img)
    ...
    def load_images(self):
        ...
        return [self.load_and_validate_image(path) for path in self.paths]  #所有照片numpy集合
...
class RowPairCalculator:
    ...
    def get_row_pair(cls, ph:PathHandler):
        ...
        drow = []
        for i in range(1,5): 
            drow += cls.compute_tp_fn_fp(dp, dt, i) #求一张照片每一类的TP之和 FN之和 FP之和
        # print('-----------------------\n',drow)
        return lrow, drow
...
class XviewMetrics:
    ...
    def get_path_handlers(self):
        self.path_handlers = []
        tload = os.listdir('./xview/test/OUT/')
        for path in tqdm(tload):
            name = path.rstrip('.png').split('_')
            print(name)
            pre_post_type, img_id = name[0], name[1]
            # assert pre_post_type in ['pre', 'post'], f"type must have 'pre' or 'post' in filename, got {path}"
            if pre_post_type == 'test': # localization or damage is fine here
...
if __name__ == '__main__':
    ...
    XviewMetrics.compute_score('./output-20221030-xviewalldata-cutmix-2-CONV','./xview/test/OUT', './out_fp_20221030-xviewalldata-cutmix-2-CONV')
    # xm = XviewMetrics('./output-20221003-ghostallSNUNet-twosam-xviewall-fiveclass','./xviewalldata-fiveclass/test/OUT')